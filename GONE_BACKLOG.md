...
> ⚠️ Доставка через GitHub Actions УБРАНА (29.06.2026): 23 agentic audit-workflow удалены.
> Задачи теперь делаются Claude в терминале по приоритету, не авто-через-Actions.
> Не трогать архитектурные инварианты — список в CLAUDE.md и GONE_AGENT_CONTEXT.md.
...
- `[x]` **Snap bolt активируется без треков** — СДЕЛАНО (проверено 29.06): `WindowSnapManager.enable()` и `dock` гардят `guard playerState?.tracks.isEmpty != true` → `snapEnabled=false`, `snapState=.off`. Ровно как требовалось.
...
_Задачи готовы к работе. Полный текст каждой — в `~/Desktop/GONE_FULL_AUDIT_TASKS.md`._
_⚠️ Старый авто-флоу (gh issue → label → Actions) МЁРТВ с 29.06. Делаются вручную Claude в терминале по приоритету._
_📋 СВЕРКА 29.06: очередь писалась под делегирование и устарела. Проверено по коду: #29 + snap-bolt — СДЕЛАНО; #20 + #25 — большей частью сделаны. Остальные P1-P2 требуют такой же сверки ПЕРЕД работой — не переписывать уже сделанное._
...
| 20 | **Critical stability:** startup freeze, bulk import 10/50/100+ треков, Clone Mode exit crash, Refresh BPM | P0 | `[~]` БОЛЬШЕЙ ЧАСТЬЮ СДЕЛАНО (проверено 29.06): clone-exit teardown (markStopped+off-main), bulk import (nonisolated scanner+express-lane), snap-старт. Startup/Refresh — подтвердить в Run |
...
| 25 | Consolidate BPM + waveform + beat-grid scheduling — единый owner, cancellation handles | P0 | `[~]` cancellation СДЕЛАНО (`analysisTasksByTrack`/`bpmBatchTask`/`waveformTask` со стораджем + `.cancel()`). «Единый owner» — открыто как рефактор |
...
| 29 | Fix Timer rule violation — `Timer.scheduledTimer` в `ClickNSView` → `.common` | P1 | `[x]` УЖЕ СДЕЛАНО — код на `Timer()`+`.common` (проверено 29.06) |
...
**Маппинг на workflows (ИСТОРИЧ. — все эти workflow удалены 29.06, оставлено для контекста какие узлы трогает задача):**
